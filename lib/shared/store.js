...
const storeReducer = (state = DEFAULT_STATE, action) => {
  switch (action.type) {
    ...
    case ACTIONS.SET_FLASH_STATE: {
      ...
      if (_.isUndefined(action.data.speed)) {
        throw errors.createError({
          title: 'Missing state speed'
        })
      }

      if ((!_.isNumber(action.data.speed) || !_.isFinite(action.data.speed)) && action.data.speed !== null) {
        throw errors.createError({
          title: `Invalid state speed: ${action.data.speed}`
        })
      }
